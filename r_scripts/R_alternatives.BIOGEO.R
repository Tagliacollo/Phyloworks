alternatives <- function (dat, group1 = "Species", group2 = "", locality = "", 
    pos = "bottomleft", ext = c(-180, 180, -60, 90)) 
{
    ...
    while (sum(sel) < n) {
        ...
        xy <- sp::SpatialPoints(xy1)
        ex <- getextent(x, y, ext)
        xlm <- ex$xlm
        ylm <- ex$ylm
        sp::plot(world, border = "gray", xlim = xlm, ylim = ylm, 
            axes = T)
        ...
        if (f1 == 7) {
            ...
            xyp <- paste("new x = ", x1, "; y = ", y1, "; Locality = ", 
                dat02$LocalityName[ffa], sep = "")
            print(xyp)
        }
